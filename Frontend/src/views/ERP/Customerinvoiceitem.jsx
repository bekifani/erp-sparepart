...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Customerinvoiceitem"));

  
 const [
    createCustomerinvoiceitem,
    { isLoading: loading, isSuccess: success, error: successError },
  ] = useCreateCustomerinvoiceitemMutation();
  const [
    updateCustomerinvoiceitem,
    { isLoading: updating, isSuccess: updated, error: updateError },
  ] = useEditCustomerinvoiceitemMutation();
  const [
    deleteCustomerinvoiceitem,
  ...
  const [data, setData] = useState([
...

    {
      ...
      formatter(cell) {
        ...
        let permission = "customerinvoiceitem";
  ...
  const [searchColumns, setSearchColumns] = useState(['customer_invoice_id', 'product_id', 'hs_code', 'brand', 'brand_code', 'oe_code', 'description', 'qty', 'price', 'amount', 'additional_note', ]);
...
additional_note : yup.string().required(t('The Additional Note field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createCustomerinvoiceitem(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Customerinvoiceitem created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Customerinvoiceitem");
    ...
    try {
      console.log('🟡 Updating customerinvoiceitem with data:', data);
      const response = await updateCustomerinvoiceitem(data);
      console.log('🟡 Update customerinvoiceitem response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Customerinvoiceitem updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Customerinvoiceitem");
      ...
      console.error('🔴 Update customerinvoiceitem error:', error);
      let errorMessage = t("Error updating Customerinvoiceitem");
    ...
    try {
      console.log('🔴 Deleting customerinvoiceitem with id:', id);
      const response = await deleteCustomerinvoiceitem(id);
      console.log('🔴 Delete customerinvoiceitem response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Customerinvoiceitem deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Customerinvoiceitem")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete customerinvoiceitem error:', error);
      let errorMessage = t("Error deleting Customerinvoiceitem");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Customerinvoiceitem")}</h2>
            </Slideover.Title>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Customerinvoiceitem")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="customerinvoiceitem-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/customerinvoiceitem"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Customerinvoiceitem"}
        />
      </Can>
    </div>
  );
