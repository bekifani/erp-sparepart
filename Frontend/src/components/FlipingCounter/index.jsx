...
const CounterFlip = () => {
  ...
  return (
    <div className="relative p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300">
      <div className="absolute top-0 right-0 -mt-4 -mr-4 bg-orange-500 rounded-full p-3">
        {stat.icon}
      </div>
            ...
            >
              {countStr.split("").map((digit, index) => {
                ...
                return (
                  <div
                    key={`${digit}-${index}`}
                    className="text-2xl md:text-5xl font-bold text-gray-900 dark:text-white border-[1px] mx-[0.2] bg-gray-950 text-white rounded-md"
                    style={{
                      position: "relative",
                      width: "0.7em",
                      height: "1.5em",
