...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Packagin"));
 ...
 const [
  ...
  const [searchColumns, setSearchColumns] = useState(['order_id', 'shipping_mark', 'invoice_no', 'qty', 'net_weight', 'total_weight', 'total_volume', 'number_of_boxes', 'order_date', 'status_id', 'internal_note', 'customer_note', ]);
...
customer_note : yup.string().required(t('The Customer Note field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createPackagin(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packagin created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Packagin");
    ...
    try {
      console.log('🟡 Updating packagin with data:', data);
      const response = await updatePackagin(data);
      console.log('🟡 Update packagin response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Packagin updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Packagin");
      ...
      console.error('🔴 Update packagin error:', error);
      let errorMessage = t("Error updating Packagin");
    ...
    try {
      console.log('🔴 Deleting packagin with id:', id);
      const response = await deletePackagin(id);
      console.log('🔴 Delete packagin response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packagin deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Packagin")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete packagin error:', error);
      let errorMessage = t("Error deleting Packagin");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Packagin")}</h2>
            </Slideover.Title>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Packagin")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="packagin-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/packagin"}
          data={data}
