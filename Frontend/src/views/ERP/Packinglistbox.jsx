...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Packinglistbox"));

  
 const [
    createPackinglistbox,
    { isLoading: loading, isSuccess: success, error: successError },
  ] = useCreatePackinglistboxMutation();
  const [
    updatePackinglistbox,
    { isLoading: updating, isSuccess: updated, error: updateError },
  ] = useEditPackinglistboxMutation();
  const [
    deletePackinglistbox,
  ...
  const [searchColumns, setSearchColumns] = useState(['packing_list_id', 'box_no', 'net_weight', 'gross_weight', 'volume', 'number_of_products', 'additional_note', ]);

  // schema
  const schema = yup
    .object({
     packing_list_id : yup.string().required(t('The Packing List Id field is required')), 
box_no : yup.string().required(t('The Box No field is required')), 
additional_note : yup.string().required(t('The Additional Note field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createPackinglistbox(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packinglistbox created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Packinglistbox");
    ...
    try {
      console.log('🟡 Updating packinglistbox with data:', data);
      const response = await updatePackinglistbox(data);
      console.log('🟡 Update packinglistbox response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Packinglistbox updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Packinglistbox");
      ...
      console.error('🔴 Update packinglistbox error:', error);
      let errorMessage = t("Error updating Packinglistbox");
    ...
    try {
      console.log('🔴 Deleting packinglistbox with id:', id);
      const response = await deletePackinglistbox(id);
      console.log('🔴 Delete packinglistbox response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packinglistbox deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Packinglistbox")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete packinglistbox error:', error);
      let errorMessage = t("Error deleting Packinglistbox");
...
return (
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Packinglistbox")}</h2>
            </Slideover.Title>
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        apiUrl={`${app_url}/api/search_packinglist`} 
        setValue={setValue} 
        variable="packing_list_id"
        customDataMapping={(item) => ({ 
          value: item.id, 
          text: item.invoice_no || item.order_id || item.qty || String(item.id) 
        })}
      />
      {errors.packing_list_id && (
        <div className="mt-2 text-danger">
          {typeof errors.packing_list_id.message === "string" &&
            errors.packing_list_id.message}
        </div>
      )}
    </div>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="box_no"
                        className={clsx({
                          "border-danger": errors.box_no,
                        })}
                        placeholder={t("Enter box_no")}
                      />
                      {errors.box_no && (
                        <div className="mt-2 text-danger">
                          {typeof errors.box_no.message === "string" &&
                            errors.box_no.message}
                        </div>
                      )}
                    </div>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Packinglistbox")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="packinglistbox-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/packinglistbox"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Packinglistbox"}
        />
      </Can>
    </div>
  );
