...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Packinglistboxitem"));

  
 const [
    createPackinglistboxitem,
    { isLoading: loading, isSuccess: success, error: successError },
  ] = useCreatePackinglistboxitemMutation();
  const [
    updatePackinglistboxitem,
    { isLoading: updating, isSuccess: updated, error: updateError },
  ] = useEditPackinglistboxitemMutation();
  const [
    deletePackinglistboxitem,
  ...
  const [data, setData] = useState([
    ...
    {
      title: t("Packing List Box Id"),
      minWidth: 200,
      field: "packing_list_box_id",
      hozAlign: "center",
...

    {
      ...
      formatter(cell) {
        ...
        let permission = "packinglistboxitem";
  ...
  const [searchColumns, setSearchColumns] = useState(['packing_list_box_id', 'product_id', 'qty', 'description', ]);

  // schema
  const schema = yup
    .object({
     packing_list_box_id : yup.string().required(t('The Packing List Box Id field is required')), 
...
description : yup.string().required(t('The Description field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createPackinglistboxitem(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packinglistboxitem created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Packinglistboxitem");
    ...
    try {
      console.log('🟡 Updating packinglistboxitem with data:', data);
      const response = await updatePackinglistboxitem(data);
      console.log('🟡 Update packinglistboxitem response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Packinglistboxitem updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Packinglistboxitem");
      ...
      console.error('🔴 Update packinglistboxitem error:', error);
      let errorMessage = t("Error updating Packinglistboxitem");
    ...
    try {
      console.log('🔴 Deleting packinglistboxitem with id:', id);
      const response = await deletePackinglistboxitem(id);
      console.log('🔴 Delete packinglistboxitem response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packinglistboxitem deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Packinglistboxitem")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete packinglistboxitem error:', error);
      let errorMessage = t("Error deleting Packinglistboxitem");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Packinglistboxitem")}</h2>
            </Slideover.Title>
      ...
      {errors.packing_list_box_id && (
        <div className="mt-2 text-danger">
          {typeof errors.packing_list_box_id.message === "string" &&
            errors.packing_list_box_id.message}
        </div>
      )}
    </div>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Packinglistboxitem")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="packinglistboxitem-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/packinglistboxitem"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Packinglistboxitem"}
        />
      </Can>
    </div>
  );
