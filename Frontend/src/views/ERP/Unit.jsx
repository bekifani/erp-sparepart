...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Unit"));
 ...
 const [
  ...
  const [searchColumns, setSearchColumns] = useState(['name', 'base_unit', 'base_value', ]);

  // schema
  const schema = yup
    .object({
     name : yup.string().required(t('The Name field is required')), 
base_unit : yup.string().required(t('The Base Unit field is required')), 
  ...
  const onCreate = async (data) => {
    try {
      ...
      setToastMessage(t("Unit created successfully."));
    } catch (error) {
      setToastMessage(t("Error creating Unit."));
    }
  ...
  const onUpdate = async (data) => {
    ...
    try {
      ...
      setToastMessage(t('Unit updated successfully'));
      setRefetch(true)
    } catch (error) {
      setShowUpdateModal(true)
      setToastMessage(t('Unit deletion failed'));
    }
    basicStickyNotification.current?.showToast();
  };
      ...
      >
        <Slideover.Panel className="  text-center overflow-y-auto max-h-[110vh]">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Unit")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="base_unit"
                        className={clsx({
                          "border-danger": errors.base_unit,
                        })}
                        placeholder={t("Enter base_unit")}
                      />
                      {errors.base_unit && (
                        <div className="mt-2 text-danger">
                          {typeof errors.base_unit.message === "string" &&
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="base_value"
                        className={clsx({
                          "border-danger": errors.base_value,
                        })}
                        placeholder={t("Enter base_value")}
                      />
                      {errors.base_value && (
                        <div className="mt-2 text-danger">
                          {typeof errors.base_value.message === "string" &&
                            errors.base_value.message}
                        </div>
                      )}
                    </div>
      ...
      >
        <Slideover.Panel className="  text-center overflow-y-auto max-h-[110vh]">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Unit")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative">
              <div className="relative">
                {loading || updating || deleting ? (
