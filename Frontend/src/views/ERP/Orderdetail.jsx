...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Orderdetail"));
 ...
 const [
  ...
  const [searchColumns, setSearchColumns] = useState(['order_id', 'product_id', 'unit_price', 'qty', 'line_total', 'arrival_time', 'status_id', 'additional_note', ]);
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Orderdetail created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Orderdetail");
    ...
    try {
      console.log('🟡 Updating orderdetail with data:', data);
      const response = await updateOrderdetail(data);
      console.log('🟡 Update orderdetail response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Orderdetail updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Orderdetail");
      ...
      console.error('🔴 Update orderdetail error:', error);
      let errorMessage = t("Error updating Orderdetail");
    ...
    try {
      console.log('🔴 Deleting orderdetail with id:', id);
      const response = await deleteOrderdetail(id);
      console.log('🔴 Delete orderdetail response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Orderdetail deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Orderdetail")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete orderdetail error:', error);
      let errorMessage = t("Error deleting Orderdetail");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Orderdetail")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch apiUrl={`${app_url}/api/search_productstatus`} setValue={setValue} variable="status_id" customDataMapping={(item) => ({ value: item.id, text: item.status_name_en || item.status_key || String(item.id) })}/>
      {errors.status_id && (
        <div className="mt-2 text-danger">
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Orderdetail")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="orderdetail-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/orderdetail"}
          data={data}
