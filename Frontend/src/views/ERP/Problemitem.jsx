...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Problemitem"));
 ...
 const [
  ...
  const [searchColumns, setSearchColumns] = useState(['problem_id', 'product_id', 'supplier_code', 'brand', 'brand_code', 'oe_code', 'description', 'qty', 'box_name', 'purchase_price', 'customer_price', 'problem_type', 'solution_type', ]);

  // schema
  const schema = yup
    .object({
     problem_id : yup.string().required(t('The Problem Id field is required')), 
...
solution_type : yup.string().required(t('The Solution Type field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createProblemitem(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Problemitem created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Problemitem");
    ...
    try {
      console.log('🟡 Updating problemitem with data:', data);
      const response = await updateProblemitem(data);
      console.log('🟡 Update problemitem response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Problemitem updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Problemitem");
      ...
      console.error('🔴 Update problemitem error:', error);
      let errorMessage = t("Error updating Problemitem");
    ...
    try {
      console.log('🔴 Deleting problemitem with id:', id);
      const response = await deleteProblemitem(id);
      console.log('🔴 Delete problemitem response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Problemitem deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Problemitem")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete problemitem error:', error);
      let errorMessage = t("Error deleting Problemitem");
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="customer_price"
                        className={clsx({
                          "border-danger": errors.customer_price,
                        })}
                        placeholder={t("Enter customer_price")}
                      />
                      {errors.customer_price && (
                        <div className="mt-2 text-danger">
                          {typeof errors.customer_price.message === "string" &&
...
<div className="mt-3 input-form">
      ...
      <Can permission="problemitem-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/problemitem"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Problemitem"}
        />
      </Can>
    </div>
  );
