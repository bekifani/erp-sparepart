...
function FileoperationAddProductNames({ onSuccess, onError, onRefresh, onDataChange }) {
  ...
  const handleFileUpload = async (acceptedFiles) => {
    ...
    console.log('Product Names: Setting unsaved data to true');
    ...
    try {
      ...
      formData.append('operation_type', 'product_names');

      const response = await axios.post(
        `${app_url}/api/fileoperation/validate-product-names`,
        formData,
        {
          headers: {
  ...
  const handleProcessImport = async (removeDuplicates = false) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${app_url}/api/fileoperation/import-product-names`, {
        valid_rows: validationResult.valid_rows,
        file_name: uploadedFile?.name || 'product_names_import.xlsx'
      }, {
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const handleExportInvalidRows = () => {
    ...
    link.setAttribute("download", "invalid_product_names.csv");
  ...
  return (
    <div className="bg-white">
      <div className="p-6">
        <div className="mb-6">
          <h3 className="text-lg font-medium mb-1">{t("Add New Product Names")}</h3>
          <p className="text-sm text-gray-600 mb-4">{t("Import product names data")}</p>
          
          {/* Required Columns */}
          <div className="mb-6">
            <div className="text-sm font-medium text-gray-700 mb-2">{t("Required Columns")}</div>
            <div className="flex gap-4 mb-2">
              <a href="#" className="text-blue-600 hover:text-blue-800 text-sm">{t("HS Code")}</a>
              <a href="#" className="text-blue-600 hover:text-blue-800 text-sm">{t("Name AZ")}</a>
              <a href="#" className="text-blue-600 hover:text-blue-800 text-sm">{t("Description EN")}</a>
              <a href="#" className="text-blue-600 hover:text-blue-800 text-sm">{t("Name RU")}</a>
              <a href="#" className="text-blue-600 hover:text-blue-800 text-sm">{t("Name CN")}</a>
              <a href="#" className="text-blue-600 hover:text-blue-800 text-sm">{t("Categories")}</a>
            </div>
            <div className="text-sm text-blue-600">
              <strong>{t("Validation")}:</strong> {t("Category vs. Categories; Product names must NOT already exist")}
            </div>
          </div>
        </div>
...
export default FileoperationAddProductNames;
