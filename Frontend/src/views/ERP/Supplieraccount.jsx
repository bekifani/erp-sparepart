...
function index_main() {
 ...
 const [
...

    {
      ...
      formatter(cell) {
        ...
        addPayment.addEventListener("click", function () {
          ...
          setSelectedSupplier({
            id: data.supplier_id,
            name: data.supplier_name || data.supplier?.company_name || 'Unknown Supplier'
          });
        ...
        let permission = "supplieraccount";
  ...
  const [searchColumns, setSearchColumns] = useState(['date', 'transaction_number', 'invoice_id', 'user_id', 'supplier_id', 'amount', 'invoice', 'payment_status', 'account_type_id', 'payment_note_id', 'picture_url', 'additional_note', 'balance', ]);

  // schema
  const schema = yup
    .object({
     transaction_number : yup.string().required(t('The Transaction Number field is required')), 
invoice_id : yup.string().required(t('The Invoice Id field is required')), 
...
invoice : yup.string().required(t('The Invoice field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplieraccount created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Supplieraccount");
    ...
    try {
      console.log('🟡 Updating supplieraccount with data:', data);
      const response = await updateSupplieraccount(data);
      console.log('🟡 Update supplieraccount response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Supplieraccount updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Supplieraccount");
      ...
      console.error('🔴 Update supplieraccount error:', error);
      let errorMessage = t("Error updating Supplieraccount");
    ...
    try {
      console.log('🔴 Deleting supplieraccount with id:', id);
      const response = await deleteSupplieraccount(id);
      console.log('🔴 Delete supplieraccount response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplieraccount deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Supplieraccount")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete supplieraccount error:', error);
      let errorMessage = t("Error deleting Supplieraccount");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Supplieraccount")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        className={clsx({
                          "border-danger": errors.transaction_number,
                        })}
                        placeholder={t("Enter transaction_number")}
                      />
                      {errors.transaction_number && (
                        <div className="mt-2 text-danger">
                          {typeof errors.transaction_number.message === "string" &&
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="invoice"
                        className={clsx({
                          "border-danger": errors.invoice,
                        })}
                        placeholder={t("Enter invoice")}
                      />
                      {errors.invoice && (
                        <div className="mt-2 text-danger">
                          {typeof errors.invoice.message === "string" &&
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Supplieraccount")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="supplieraccount-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/supplieraccount"}
          data={data}
