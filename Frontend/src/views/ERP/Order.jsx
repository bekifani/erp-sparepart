  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        ...
        let errorMessage = t("Error creating Order");
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        ...
        let errorMessage = t("Error updating Order");
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Order")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete order error:', error);
      let errorMessage = t("Error deleting Order");
...
<div className="mt-3 input-form">
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        ...
        customDataMapping={(item) => ({
          value: item.id,
          text: item.status_name_en || item.status_key || item.description || String(item.id)
        })}
      />
      {errors.status_id && (
        <div className="mt-2 text-danger">
...
<div className="mt-3 input-form">
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        apiUrl={`${app_url}/api/search_compan`} 
        setValue={setValue} 
        variable="company_id"
        customDataMapping={(item) => ({
          value: item.id,
          text: item.company_name || item.our_ref || item.email || String(item.id)
        })}
      />
      {errors.company_id && (
        <div className="mt-2 text-danger">
