...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Basketfile"));
 ...
 const [
  ...
  const [data, setData] = useState([
   ...
   
    {
      ...
      download: true,
      formatter(cell) {
      return getMiniDisplay(cell.getData().file)
      }
    },
  ...
  const schema = yup
    .object({
     file : yup.string().required(t('The File field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createBasketfile(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Basketfile created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Basketfile");
    ...
    try {
      console.log('🟡 Updating basketfile with data:', data);
      const response = await updateBasketfile(data);
      console.log('🟡 Update basketfile response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Basketfile updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Basketfile");
      ...
      console.error('🔴 Update basketfile error:', error);
      let errorMessage = t("Error updating Basketfile");
    ...
    try {
      console.log('🔴 Deleting basketfile with id:', id);
      const response = await deleteBasketfile(id);
      console.log('🔴 Delete basketfile response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Basketfile deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Basketfile")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete basketfile error:', error);
      let errorMessage = t("Error deleting Basketfile");
...
return (
    <div>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Basketfile")}</h2>
            </Slideover.Title>
          ...
          <div className="w-full ">
              <FileUpload endpoint={upload_url} type="image/*" className="w-full " setUploadedURL={setUploadFile}/>
          </div>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Basketfile")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
      ...
      <Can permission="basketfile-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/basketfile"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Basketfile"}
        />
      </Can>
    </div>
  );
