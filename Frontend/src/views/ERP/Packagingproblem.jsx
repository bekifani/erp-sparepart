...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Packagingproblem"));

  
 const [
    createPackagingproblem,
    { isLoading: loading, isSuccess: success, error: successError },
  ] = useCreatePackagingproblemMutation();
  const [
    updatePackagingproblem,
    { isLoading: updating, isSuccess: updated, error: updateError },
  ] = useEditPackagingproblemMutation();
  const [
    deletePackagingproblem,
  ...
  const [data, setData] = useState([
...

    {
      ...
      formatter(cell) {
        ...
        let permission = "packagingproblem";
  ...
  const [searchColumns, setSearchColumns] = useState(['package_id', 'problems', 'additional_note', ]);

  // schema
  const schema = yup
    .object({
     package_id : yup.string().required(t('The Package Id field is required')), 
problems : yup.string().required(t('The Problems field is required')), 
additional_note : yup.string().required(t('The Additional Note field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createPackagingproblem(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packagingproblem created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Packagingproblem");
    ...
    try {
      console.log('🟡 Updating packagingproblem with data:', data);
      const response = await updatePackagingproblem(data);
      console.log('🟡 Update packagingproblem response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Packagingproblem updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Packagingproblem");
      ...
      console.error('🔴 Update packagingproblem error:', error);
      let errorMessage = t("Error updating Packagingproblem");
    ...
    try {
      console.log('🔴 Deleting packagingproblem with id:', id);
      const response = await deletePackagingproblem(id);
      console.log('🔴 Delete packagingproblem response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packagingproblem deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Packagingproblem")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete packagingproblem error:', error);
      let errorMessage = t("Error deleting Packagingproblem");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Packagingproblem")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="problems"
                        className={clsx({
                          "border-danger": errors.problems,
                        })}
                        placeholder={t("Enter problems")}
                      />
                      {errors.problems && (
                        <div className="mt-2 text-danger">
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Packagingproblem")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="packagingproblem-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/packagingproblem"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Packagingproblem"}
        />
      </Can>
    </div>
  );
