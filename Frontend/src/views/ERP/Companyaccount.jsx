...
additional_note : yup.string().required(t('The Additional Note field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createCompanyaccount(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Companyaccount created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Companyaccount");
    ...
    try {
      console.log('🟡 Updating companyaccount with data:', data);
      const response = await updateCompanyaccount(data);
      console.log('🟡 Update companyaccount response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Companyaccount updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Companyaccount");
      ...
      console.error('🔴 Update companyaccount error:', error);
      let errorMessage = t("Error updating Companyaccount");
    ...
    try {
      console.log('🔴 Deleting companyaccount with id:', id);
      const response = await deleteCompanyaccount(id);
      console.log('🔴 Delete companyaccount response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Companyaccount deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Companyaccount")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete companyaccount error:', error);
      let errorMessage = t("Error deleting Companyaccount");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Companyaccount")}</h2>
            </Slideover.Title>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Companyaccount")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="companyaccount-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/companyaccount"}
          data={data}
