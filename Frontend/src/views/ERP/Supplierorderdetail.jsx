  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        ...
        let errorMessage = t("Error creating Supplierorderdetail");
    ...
    try {
      console.log('🟡 Updating supplierorderdetail with data:', data);
      const response = await updateSupplierorderdetail(data);
      console.log('🟡 Update supplierorderdetail response:', response);
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        ...
        let errorMessage = t("Error updating Supplierorderdetail");
      ...
      console.error('🔴 Update supplierorderdetail error:', error);
      let errorMessage = t("Error updating Supplierorderdetail");
    ...
    try {
      console.log('🔴 Deleting supplierorderdetail with id:', id);
      const response = await deleteSupplierorderdetail(id);
      console.log('🔴 Delete supplierorderdetail response:', response);
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Supplierorderdetail")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete supplierorderdetail error:', error);
      let errorMessage = t("Error deleting Supplierorderdetail");
...
return (
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        apiUrl={`${app_url}/api/search_supplierorder`} 
        setValue={setValue} 
        variable="supplier_order_id"
        customDataMapping={(item) => ({ value: item.id, text: item.supplier_invoice_no || item.invoice_no || String(item.id) })}
      />
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        apiUrl={`${app_url}/api/search_orderdetail`} 
        setValue={setValue} 
        variable="order_detail_id"
        customDataMapping={(item) => ({ value: item.id, text: item.invoice_no || item.order_number || String(item.id) })}
      />
...
<div className="mt-3 input-form">
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        ...
        setValue={setValue} 
        variable="status_id"
        customDataMapping={(item) => ({ value: item.id, text: item.status_name_en || item.status_key || item.status_name_az || String(item.id) })}
      />
      {errors.status_id && (
        <div className="mt-2 text-danger">
