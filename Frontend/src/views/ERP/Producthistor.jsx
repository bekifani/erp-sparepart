...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Producthistor"));
 ...
 const [
  ...
  const [searchColumns, setSearchColumns] = useState(['date', 'product_id', 'customer_id', ]);
...
customer_id : yup.string().required(t('The Customer Id field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createProducthistor(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Producthistor created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Producthistor");
    ...
    try {
      console.log('🟡 Updating producthistor with data:', data);
      const response = await updateProducthistor(data);
      console.log('🟡 Update producthistor response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Producthistor updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Producthistor");
      ...
      console.error('🔴 Update producthistor error:', error);
      let errorMessage = t("Error updating Producthistor");
    ...
    try {
      console.log('🔴 Deleting producthistor with id:', id);
      const response = await deleteProducthistor(id);
      console.log('🔴 Delete producthistor response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Producthistor deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Producthistor")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete producthistor error:', error);
      let errorMessage = t("Error deleting Producthistor");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Producthistor")}</h2>
            </Slideover.Title>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Producthistor")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
   ...
   <div className="mt-3 input-form">
      ...
      <Can permission="producthistor-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/producthistor"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Producthistor"}
        />
      </Can>
    </div>
  );
