...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Supplierpricingrule"));
 ...
 const [
  ...
  const [data, setData] = useState([
...

    {
      ...
      formatter(cell) {
        ...
        let permission = "supplierpricingrule";
  ...
  const [searchColumns, setSearchColumns] = useState(['supplier_id', 'adjustment_percent', 'adjustment_type', 'scope', 'active', 'valid_from', 'valid_to', ]);

  // schema
  const schema = yup
    .object({
     supplier_id : yup.string().required(t('The Supplier Id field is required')), 
adjustment_type : yup.string().required(t('The Adjustment Type field is required')), 
scope : yup.string().required(t('The Scope field is required')), 
active : yup.string().required(t('The Active field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createSupplierpricingrule(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplierpricingrule created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Supplierpricingrule");
    ...
    try {
      console.log('🟡 Updating supplierpricingrule with data:', data);
      const response = await updateSupplierpricingrule(data);
      console.log('🟡 Update supplierpricingrule response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Supplierpricingrule updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Supplierpricingrule");
      ...
      console.error('🔴 Update supplierpricingrule error:', error);
      let errorMessage = t("Error updating Supplierpricingrule");
    ...
    try {
      console.log('🔴 Deleting supplierpricingrule with id:', id);
      const response = await deleteSupplierpricingrule(id);
      console.log('🔴 Delete supplierpricingrule response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplierpricingrule deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Supplierpricingrule")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete supplierpricingrule error:', error);
      let errorMessage = t("Error deleting Supplierpricingrule");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Supplierpricingrule")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
                      ...
                      >
                        {t("Adjustment Percent")}
                      </FormLabel>
                      <FormInput
                        {...register("adjustment_percent")}
                        id="validation-form-1"
                        type="number"
                        name="adjustment_percent"
                        className={clsx({
                          "border-danger": errors.adjustment_percent,
                        })}
                        placeholder={t("Enter adjustment_percent")}
                      />
                      {errors.adjustment_percent && (
                        <div className="mt-2 text-danger">
                          {typeof errors.adjustment_percent.message === "string" &&
                            errors.adjustment_percent.message}
                        </div>
                      )}
                    </div>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        className={clsx({
                          "border-danger": errors.adjustment_type,
                        })}
                        placeholder={t("Enter adjustment_type")}
                      />
                      {errors.adjustment_type && (
                        <div className="mt-2 text-danger">
                          {typeof errors.adjustment_type.message === "string" &&
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="scope"
                        className={clsx({
                          "border-danger": errors.scope,
                        })}
                        placeholder={t("Enter scope")}
                      />
                      {errors.scope && (
                        <div className="mt-2 text-danger">
                          {typeof errors.scope.message === "string" &&
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="valid_from"
                        className={clsx({
                          "border-danger": errors.valid_from,
                        })}
                        placeholder={t("Enter valid_from")}
                      />
                      {errors.valid_from && (
                        <div className="mt-2 text-danger">
                          {typeof errors.valid_from.message === "string" &&
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="valid_to"
                        className={clsx({
                          "border-danger": errors.valid_to,
                        })}
                        placeholder={t("Enter valid_to")}
                      />
                      {errors.valid_to && (
                        <div className="mt-2 text-danger">
                          {typeof errors.valid_to.message === "string" &&
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Supplierpricingrule")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="supplierpricingrule-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/supplierpricingrule"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Supplierpricingrule"}
        />
      </Can>
    </div>
  );
