...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Productimage"));
 ...
 const [
  ...
  const [data, setData] = useState([
...

    {
      ...
      download: true,
      formatter(cell) {
      return getMiniDisplay(cell.getData().image_url)
      }
    },
  ...
  const [searchColumns, setSearchColumns] = useState(['product_information_id', 'is_primary', 'uploaded_at', ]);
...
is_primary : yup.string().required(t('The Is Primary field is required')), 
  ...
  const {
    ...
    resolver: yupResolver(schema),
          ...
          const setUploadImage_url  = (value) => {
  ...
  const onCreate = async (data) => {
    try {
      ...
      setToastMessage(t("Productimage created successfully."));
    } catch (error) {
      setToastMessage(t("Error creating Productimage."));
    }
  ...
  const onUpdate = async (data) => {
    ...
    try {
      ...
      setToastMessage(t('Productimage updated successfully'));
      setRefetch(true)
    } catch (error) {
      setShowUpdateModal(true)
      setToastMessage(t('Productimage deletion failed'));
    }
    basicStickyNotification.current?.showToast();
  };
  ...
  const onDelete = async () => {
    ...
    try {
        const response = deleteProductimage(id);
        setToastMessage(t("Productimage deleted successfully."));
        setRefetch(true);
      }
    catch (error) {
      setToastMessage(t("Error deleting Productimage."));
    }
    basicStickyNotification.current?.showToast();
  };    
      ...
      {errors.product_information_id && (
        <div className="mt-2 text-danger">
          ...
          <div className="w-full ">
              <FileUpload endpoint={upload_url} type="image/*" className="w-full " setUploadedURL={setUploadImage_url}/>
          </div>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        {...register("uploaded_at")}
                        id="validation-form-1"
                        type="date"
                        name="uploaded_at"
                        className={clsx({
                          "border-danger": errors.uploaded_at,
                        })}
                        placeholder={t("Enter uploaded_at")}
                      />
                      {errors.uploaded_at && (
                        <div className="mt-2 text-danger">
                          {typeof errors.uploaded_at.message === "string" &&
                            errors.uploaded_at.message}
                        </div>
                      )}
                    </div>
...
<div className="mt-3 input-form">
      ...
      <Can permission="productimage-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/productimage"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Productimage"}
        />
      </Can>
    </div>
  );
