...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Paymentnote"));
 ...
 const [
  ...
  const [searchColumns, setSearchColumns] = useState(['note', 'note_ch', 'note_az', ]);

  // schema
  const schema = yup
    .object({
     note : yup.string().required(t('The Note field is required')), 
note_ch : yup.string().required(t('The Note Ch field is required')), 
note_az : yup.string().required(t('The Note Az field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createPaymentnote(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Paymentnote created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Paymentnote");
    ...
    try {
      console.log('🟡 Updating paymentnote with data:', data);
      const response = await updatePaymentnote(data);
      console.log('🟡 Update paymentnote response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Paymentnote updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Paymentnote");
      ...
      console.error('🔴 Update paymentnote error:', error);
      let errorMessage = t("Error updating Paymentnote");
    ...
    try {
      console.log('🔴 Deleting paymentnote with id:', id);
      const response = await deletePaymentnote(id);
      console.log('🔴 Delete paymentnote response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Paymentnote deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Paymentnote")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete paymentnote error:', error);
      let errorMessage = t("Error deleting Paymentnote");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Paymentnote")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="note_ch"
                        className={clsx({
                          "border-danger": errors.note_ch,
                        })}
                        placeholder={t("Enter note_ch")}
                      />
                      {errors.note_ch && (
                        <div className="mt-2 text-danger">
                          {typeof errors.note_ch.message === "string" &&
                            errors.note_ch.message}
                        </div>
                      )}
                    </div>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="note_az"
                        className={clsx({
                          "border-danger": errors.note_az,
                        })}
                        placeholder={t("Enter note_az")}
                      />
                      {errors.note_az && (
                        <div className="mt-2 text-danger">
                          {typeof errors.note_az.message === "string" &&
                            errors.note_az.message}
                        </div>
                      )}
                    </div>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Paymentnote")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="paymentnote-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/paymentnote"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Paymentnote"}
        />
      </Can>
    </div>
  );
