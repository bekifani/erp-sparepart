...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Supplierinvoice"));
 ...
 const [
  ...
  const [data, setData] = useState([
    ...
    {
      title: t("Total Products Qty"),
      minWidth: 200,
      field: "total_products_qty",
      hozAlign: "center",
      headerHozAlign: "center",
      vertAlign: "middle",
      print: true,
...

    {
      ...
      formatter(cell) {
        ...
        let permission = "supplierinvoice";
  ...
  const [searchColumns, setSearchColumns] = useState(['invoice_no', 'supplier_id', 'customer_id', 'user_id', 'arrival_time', 'shipping_date', 'shipped_date', 'shipping_mark', 'supplier_code', 'total_products_qty', 'total_qty', 'total_amount', 'total_weight', 'total_volume', 'total_ctn', 'discount', 'deposit', 'extra_expenses', 'supplier_debt', 'balance', 'additional_note', 'status', ]);
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplierinvoice created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Supplierinvoice");
    ...
    try {
      console.log('🟡 Updating supplierinvoice with data:', data);
      const response = await updateSupplierinvoice(data);
      console.log('🟡 Update supplierinvoice response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Supplierinvoice updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Supplierinvoice");
      ...
      console.error('🔴 Update supplierinvoice error:', error);
      let errorMessage = t("Error updating Supplierinvoice");
    ...
    try {
      console.log('🔴 Deleting supplierinvoice with id:', id);
      const response = await deleteSupplierinvoice(id);
      console.log('🔴 Delete supplierinvoice response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplierinvoice deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Supplierinvoice")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete supplierinvoice error:', error);
      let errorMessage = t("Error deleting Supplierinvoice");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Supplierinvoice")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
                      <FormLabel
                        ...
                        className="flex justify-start items-start flex-col w-full sm:flex-row"
                      >
                        {t("Total Products Qty")}
                      </FormLabel>
                      <FormInput
                        {...register("total_products_qty")}
                        id="validation-form-1"
                        type="number"
                        name="total_products_qty"
                        className={clsx({
                          "border-danger": errors.total_products_qty,
                        })}
                        placeholder={t("Enter total_products_qty")}
                      />
                      {errors.total_products_qty && (
                        <div className="mt-2 text-danger">
                          {typeof errors.total_products_qty.message === "string" &&
                            errors.total_products_qty.message}
                        </div>
                      )}
                    </div>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="total_ctn"
                        className={clsx({
                          "border-danger": errors.total_ctn,
                        })}
                        placeholder={t("Enter total_ctn")}
                      />
                      {errors.total_ctn && (
                        <div className="mt-2 text-danger">
                          {typeof errors.total_ctn.message === "string" &&
                            errors.total_ctn.message}
                        </div>
                      )}
                    </div>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        {...register("supplier_debt")}
                        id="validation-form-1"
                        type="number"
                        name="supplier_debt"
                        className={clsx({
                          "border-danger": errors.supplier_debt,
                        })}
                        placeholder={t("Enter supplier_debt")}
                      />
                      {errors.supplier_debt && (
                        <div className="mt-2 text-danger">
                          {typeof errors.supplier_debt.message === "string" &&
                            errors.supplier_debt.message}
                        </div>
                      )}
                    </div>
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Supplierinvoice")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="supplierinvoice-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/supplierinvoice"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Supplierinvoice"}
        />
      </Can>
    </div>
  );
