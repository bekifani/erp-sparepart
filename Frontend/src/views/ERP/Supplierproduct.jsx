...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Supplierproduct"));
 ...
 const [
  ...
  const [data, setData] = useState([
...

    {
      ...
      formatter(cell) {
        ...
        let permission = "supplierproduct";
  ...
  const [searchColumns, setSearchColumns] = useState(['supplier_id', 'product_id', 'is_primary', ]);
...
is_primary : yup.string().required(t('The Is Primary field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createSupplierproduct(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplierproduct created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Supplierproduct");
    ...
    try {
      console.log('🟡 Updating supplierproduct with data:', data);
      const response = await updateSupplierproduct(data);
      console.log('🟡 Update supplierproduct response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Supplierproduct updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Supplierproduct");
      ...
      console.error('🔴 Update supplierproduct error:', error);
      let errorMessage = t("Error updating Supplierproduct");
    ...
    try {
      console.log('🔴 Deleting supplierproduct with id:', id);
      const response = await deleteSupplierproduct(id);
      console.log('🔴 Delete supplierproduct response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Supplierproduct deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Supplierproduct")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete supplierproduct error:', error);
      let errorMessage = t("Error deleting Supplierproduct");
...
return (
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Supplierproduct")}</h2>
            </Slideover.Title>
 ...
 <div className="mt-3 input-form">
      ...
      {errors.is_primary && (
        <div className="mt-2 text-danger">
          {typeof errors.is_primary.message === "string" &&
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Supplierproduct")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
 ...
 <div className="mt-3 input-form">
      ...
      <Can permission="supplierproduct-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/supplierproduct"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Supplierproduct"}
        />
      </Can>
    </div>
  );
