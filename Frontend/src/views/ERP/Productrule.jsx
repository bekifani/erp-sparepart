...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Productrule"));
 ...
 const [
  ...
  const [searchColumns, setSearchColumns] = useState(['customer_id', 'product_id', 'fixed_price', 'adjustment_percent', 'qty', 'note', ]);
...
note : yup.string().required(t('The Note field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      const response = await createProductrule(data);
      console.log('🟡 API response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Productrule created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Productrule");
    ...
    try {
      console.log('🟡 Updating productrule with data:', data);
      const response = await updateProductrule(data);
      console.log('🟡 Update productrule response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Productrule updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Productrule");
      ...
      console.error('🔴 Update productrule error:', error);
      let errorMessage = t("Error updating Productrule");
    ...
    try {
      console.log('🔴 Deleting productrule with id:', id);
      const response = await deleteProductrule(id);
      console.log('🔴 Delete productrule response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Productrule deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Productrule")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete productrule error:', error);
      let errorMessage = t("Error deleting Productrule");
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Productrule")}</h2>
            </Slideover.Title>
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        name="fixed_price"
                        className={clsx({
                          "border-danger": errors.fixed_price,
                        })}
                        placeholder={t("Enter fixed_price")}
                      />
                      {errors.fixed_price && (
                        <div className="mt-2 text-danger">
                          {typeof errors.fixed_price.message === "string" &&
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Productrule")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="productrule-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/productrule"}
          data={data}
