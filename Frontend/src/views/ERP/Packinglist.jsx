...
function index_main() {
  ...
  const [confirmationMessage, setConfirmationMessage] =
    useState(t("Are you Sure Do You want to Delete Packinglist"));
 ...
 const [
  ...
  const handleCompanyChange = (companyId) => {
    ...
    if (companyId && companiesData?.data?.data) {
      ...
      // Auto-populate company fields if they exist in the form
      if (company) {
  ...
  const [searchColumns, setSearchColumns] = useState(['order_id', 'invoice_no', 'qty', 'net_weight', 'total_weight', 'total_volume', 'number_of_boxes', 'order_date', 'order_period', 'shipping_date', 'status_id', 'additional_note', ]);

  // schema
  const schema = yup
    .object({
     order_id : yup.string().required(t('The Order Id field is required')), 
  ...
  const getMiniDisplay = (url) => {
    ...
    try {
      ...
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packinglist created successfully."));
        setRefetch(true);
        ...
        let errorMessage = t("Error creating Packinglist");
    ...
    try {
      console.log('🟡 Updating packinglist with data:', data);
      const response = await updatePackinglist(data);
      console.log('🟡 Update packinglist response:', response);
        
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t('Packinglist updated successfully'));
        setRefetch(true);
        setShowUpdateModal(false);
      } else {
        // Handle validation errors with comprehensive error extraction
        let errorMessage = t("Error updating Packinglist");
    ...
    try {
      console.log('🔴 Deleting packinglist with id:', id);
      const response = await deletePackinglist(id);
      console.log('🔴 Delete packinglist response:', response);
      
      if (response && (response.success === true || response.data?.success === true)) {
        setToastMessage(t("Packinglist deleted successfully."));
        setRefetch(true);
      } else {
        ...
        if (response?.error?.data?.data?.errors) {
          ...
          setToastMessage(`${t("Error deleting Packinglist")}: ${errorMsg}`);
        }
        console.error('🔴 Delete failed with response:', response);
      }
    } catch (error) {
      console.error('🔴 Delete packinglist error:', error);
      let errorMessage = t("Error deleting Packinglist");
...
return (
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onCreate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Add New Packinglist")}</h2>
            </Slideover.Title>
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        apiUrl={`${app_url}/api/search_order`} 
        setValue={setValue} 
        variable="order_id"
        customDataMapping={(item) => ({ 
          value: item.id, 
          text: item.invoice_no || item.order_number || String(item.id) 
        })}
      />
      {errors.order_id && (
...
<div className="mt-3 input-form">
                      ...
                      <FormInput
                        ...
                        className={clsx({
                          "border-danger": errors.number_of_boxes,
                        })}
                        placeholder={t("Enter number_of_boxes")}
                      />
                      {errors.number_of_boxes && (
                        <div className="mt-2 text-danger">
                          {typeof errors.number_of_boxes.message === "string" &&
                            errors.number_of_boxes.message}
                        </div>
                      )}
                    </div>
...
<div className="mt-3 input-form">
   ...
   <div className="mt-3 input-form">
      ...
      <TomSelectSearch 
        apiUrl={`${app_url}/api/search_productstatus`} 
        setValue={setValue} 
        variable="status_id"
        customDataMapping={(item) => ({ 
          value: item.id, 
          text: item.status_name_en || item.status_key || item.status_name_az || String(item.id) 
        })}
      />
      {errors.status_id && (
        <div className="mt-2 text-danger">
      ...
      >
        <Slideover.Panel className="text-center">
          <form onSubmit={handleSubmit(onUpdate)}>
            <Slideover.Title>
              <h2 className="mr-auto text-base font-medium">{t("Edit Packinglist")}</h2>
            </Slideover.Title>
            <Slideover.Description className="relative overflow-y-auto max-h-[calc(100vh-200px)]">
              <div className="relative">
                {loading || updating || deleting ? (
...
<div className="mt-3 input-form">
      ...
      <Can permission="packinglist-list">
        <TableComponent
          setShowCreateModal={setShowCreateModal}
          endpoint={app_url + "/api/packinglist"}
          data={data}
          searchColumns={searchColumns}
          refetch={refetch}
          setRefetch={setRefetch}
          permission={"Packinglist"}
        />
      </Can>
    </div>
  );
