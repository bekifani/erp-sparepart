...
function FileoperationAddProductInformation({ onSuccess, onError, onRefresh, onDataChange }) {
  ...
  const handleFileUpload = async (acceptedFiles) => {
    ...
    console.log('Product Information: Setting unsaved data to true');
    ...
    try {
      ...
      formData.append('operation_type', 'product_information');

      const response = await axios.post(
        `${app_url}/api/fileoperation/validate-product-information`,
        formData,
        {
          headers: {
  ...
  const handleProcessImport = async (removeDuplicates = false) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${app_url}/api/fileoperation/import-product-information`, {
        valid_rows: validationResult.valid_rows,
        file_name: uploadedFile?.name || 'product_information_import.xlsx'
      }, {
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const handleExportInvalidRows = () => {
    ...
    link.setAttribute("download", "invalid_product_information.csv");
  ...
  const renderDataGrid = () => {
    ...
    // Get dynamic column headers from the first row of data
    ...
    return (
      <div className="mt-6">
        ...
        {/* Column headers with dropdowns - Dynamic based on Excel columns */}
        <div className="flex items-center gap-0 mb-4 bg-gray-50 border border-gray-200 rounded-t-lg overflow-x-auto">
          {headers.map((header, index) => (
            <div key={index} className="relative flex-none min-w-32 px-4 py-3 border-r border-gray-200">
                  ...
                  return (
                    <tr key={index} className={`border-b border-gray-100 hover:bg-gray-50 ${hasRowErrors ? 'bg-red-50' : ''}`}>
                      ...
                      {row.data.map((cellData, cellIndex) => (
                        <td key={cellIndex} className="px-4 py-3 min-w-32 border-r border-gray-100">
                          <span className={hasRowErrors && cellData ? 'text-red-600 font-medium' : ''}>
                            {cellData || ''}
                          </span>
                        </td>
                      ))}
  ...
  return (
    <div className="bg-white">
      <div className="p-6">
        <div className="mb-6">
          <h3 className="text-lg font-medium mb-1">{t("Add Product Information")}</h3>
          <p className="text-sm text-gray-600 mb-4">{t("Import product information data")}</p>
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-4 mb-2">
              ...
              <a href="#" className="text-blue-600 hover:text-blue-800 text-sm">{t("Brand Code")}</a>
            </div>
            <div className="text-sm text-blue-600">
              <strong>{t("Validation")}:</strong> {t("Brand/Brand Code vs. Products table")}
            </div>
          </div>
        </div>
...
export default FileoperationAddProductInformation;